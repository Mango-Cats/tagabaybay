...
//! finding the corresponding vowel phoneme at that count.
...
/// Returns `None` if not applicable or alignment fails.
pub fn phonetic_replacements(ctx: &Cursor, config: &AdaptationConfig) -> Option<(Vec<FilipinoGrapheme>, usize)> {
    ...
    // Count how many vowel graphemes appear before the current position
    let vowel_index = ctx
        .graphemes
        .iter()
        .take(ctx.index)
        .filter(|g| g.is_unpredictable_variant())
        .count();

    let phoneme = find_nth_vowel_phoneme(&ctx.phonemes, vowel_index)?;
    ...
    let consumed = if is_diphthong && next_is_vowel {
        2
    } else {
        1
    };

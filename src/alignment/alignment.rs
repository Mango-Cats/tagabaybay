...
fn is_double_vowel(ctx: &Cursor, p: &Vec<IPASymbol>, p_index: usize) -> bool {
    ...
    let current_vowel =
        current.is_vowel() || current == SourceGrapheme::W || current == SourceGrapheme::Y;
...
fn is_case_ld(ctx: &Cursor, p: &Vec<IPASymbol>, p_index: usize) -> bool {
    if ctx.current_grapheme() == SourceGrapheme::L && ctx.next_grapheme() == Some(SourceGrapheme::D)
    {
...
fn handle_phonemes(
    ...
    p: &Vec<IPASymbol>,
    p_index: &mut usize,
) -> Vec<Option<IPASymbol>> {
    let current_grapheme = ctx.current_grapheme();
    let next_grapheme = ctx.next_grapheme();
    let prev_grapheme = ctx.prev_grapheme();

    let next_grapheme_is_vowel = next_grapheme.clone().map(|g| g.is_vowel()).unwrap_or(false);

    // When H is silent, ie. honor
    if current_grapheme == SourceGrapheme::H && p[*p_index].is_vowel() {
        return vec![None];
    }
    ...
    if *p_index >= 1 {
        ...
        if prev_ph.is_diphthong()
            && (current_grapheme == SourceGrapheme::W || current_grapheme == SourceGrapheme::Y)
        {
            return vec![None];
        }
        ...
        if prev_ph == IPASymbol::OpenMidCentral && current_grapheme == SourceGrapheme::R {
            return vec![None];
        }
        ...
        if current_grapheme == SourceGrapheme::H && prev_grapheme == Some(SourceGrapheme::SC) {
            return vec![None];
        }

        if current_grapheme == SourceGrapheme::W && p[*p_index] != IPASymbol::LabialVelarApproximant
        {
            return vec![None];
        }

        if current_grapheme == SourceGrapheme::S && next_grapheme == Some(SourceGrapheme::SE) {
            return vec![None];
        }

        if current_grapheme == SourceGrapheme::E && prev_grapheme == Some(SourceGrapheme::QUE) {
            return vec![None];
        }
    ...
    *p_index += 1;
    ...
    if *p_index < p.len() {
        ...
        if current_grapheme == SourceGrapheme::X
            && next_grapheme != Some(SourceGrapheme::C)
        ...
        // of the current phoneme and /d/ is returned
            ...
            if next_ph != IPASymbol::VoicedAlveolarStop
                && (next_ph == IPASymbol::VoicedPostalveolarAffricate
                    && next_grapheme == Some(SourceGrapheme::GE))
            {
        ...
        // If grapheme is QUE pronounced /kwə/, map a vector of 3 phonemes
            ...
            *p_index += 1;
            ...
            if *p_index < p.len()
                && next_ph == IPASymbol::LabialVelarApproximant
                && p[*p_index].is_vowel()
            {
                let vec = vec![Some(ph), Some(next_ph), Some(p[*p_index].clone())];
                *p_index += 1;
...
fn free_replacement(result: &mut AlignedString) {
    ...
    for idx in 0..len {
        let prev_grapheme = result.get(idx.saturating_sub(1)).map(|(prev_g, _)| prev_g);
        let next_grapheme = result.get(idx + 1).map(|(next_g, _)| next_g);

        // Checking if the vowel being replaced is in CVC format
        let prev_is_consonant = !matches!(
            prev_grapheme,
...
fn print_aligned_string(result: &AlignedString) {
    for (index, (grapheme, phoneme_vec)) in result.iter().enumerate() {
        ...
        let phoneme_strs: Vec<String> = phoneme_vec
            .iter()

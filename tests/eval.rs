...
use tagabaybay::nativization::replacement::nativize_word;
...
fn write_report(name: &str, report: &EvalReport, timestamp: &str) -> String {
    ...
    let max_input = report.failures.iter().map(|f| f.input.len()).max().unwrap_or(10).max(10);
    let max_actual = report.failures.iter().map(|f| f.actual.len()).max().unwrap_or(10).max(10);
    let max_expected = report.failures.iter().map(|f| f.expected.len()).max().unwrap_or(10).max(10);
    ...
    if !report.failures.is_empty() {
        content.push_str("\nFailures:\n");
        content.push_str(&format!("  {}  {:<width_in$}  {:<width_act$}  {:<width_exp$}\n", 
            "#", "INPUT", "ACTUAL", "EXPECTED",
            width_in = max_input, width_act = max_actual, width_exp = max_expected));
        content.push_str(&format!("  {}  {}  {}  {}\n", 
            "-".repeat(4), "-".repeat(max_input), "-".repeat(max_actual), "-".repeat(max_expected)));
        
        for (i, f) in report.failures.iter().enumerate() {
            let (highlighted_actual, highlighted_expected) = highlight_differences(&f.actual, &f.expected);
            content.push_str(&format!("  {:>3}. {:<width_in$}  {:<width_act$}  {:<width_exp$}\n", 
                i+1, f.input, highlighted_actual, highlighted_expected,
                width_in = max_input, width_act = max_actual, width_exp = max_expected));
        }
    }
...
fn compare() {
    ...
    let worst = reports_info.iter().min_by(|a, b| a.1.partial_cmp(&b.1).unwrap()).unwrap();
    ...
    println!("└── Worst Performer {} ({:.2}%)", worst.0.replace(".csv", ""), worst.1);
    
    println!("\nReports written to:");
    for (name, accuracy, file) in reports_info {
        println!("  - {} ({:.2}%): {}", name.replace(".csv", ""), accuracy, file);
    }
}